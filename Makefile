...
MAKE_LOGFILE = /tmp/internal-dload.log
MAKE_CMD_COLOR := $(BLUE)
...
help: ## Show this menu
	...
	@echo
	@echo '    📦 Package                 dload (github.com/internal/dload)'
	@echo '    🤠 Makefile Author         Andrij Orlenko (github.com/lotyp)'
	@echo '    🏢 ${YELLOW}Org                     PHP Internal (github.com/php-internal)${RST}'
	@echo
.PHONY: help
...
phar:
	$(APP_RUNNER) sh -c "git config --global --add safe.directory /app \
		&& .phive/box validate \
		&& .phive/box compile \
		&& .phive/box info .build/phar/dload.phar \
		&& .build/phar/dload.phar"
.PHONY: phar
...
lint: lint-yaml lint-actions lint-php lint-composer lint-audit ## Runs all linting commands
.PHONY: lint
